...
const fetchNews = async (object) => {
  ...
  if (object.totalResults != undefined) {
    resultNum.innerHTML = `(${object.totalResults}) Results Found for ${search.value}`
  }
  ...
  for (let item of object.results) {
    ...
    <img src="${item.image_url}" class="min-[1500px]:max-h-[130px] mx-auto min-[1800px]:max-h-[160px] min-[1950px]:max-h-[200px]" alt="Sorry Image Cannot be loaded">    
    <h2 class="text-sky-400 text-center text-xl font-semibold mt-[${num}]">${item.title}</h2>
        <p class="mt-5 mx-8 text-center">${myArray}</p>
          <a href="${item.link}" class="place-items-center -mr-20" target="_blank"><button
            class="bg-red-500 px-8 min-[2100px]:ml-[80px] min-[1800px]:ml-[75px] min-[1600px]:ml-[70px] ml-[65px] py-3 rounded-xl mt-10 min-[1800px]:px-12 min-[1800px]:py-5 min-[1800px]:text-lg">Read
            More</button></a>
      </div>
      `
  }
